...
HashCell *create_hashcell(Key *key){
    ...
    //crée une cellule avec la clé Key
...
int hash_function(Key *key, int size){
    ...
    //retourne la valeur hachée de key dans une table de taille size
    //valeur de retour: la valeur de hachage de key->val
	if(!key){
		printf("Key non initialisé\n");
		return -1;	
	}
	float nb = (sqrtf(5)-1)/2; //on utilise le nombre d'or diminué de 1
...
HashTable *create_hashtable(CellKey *keys, int size){
    ...
    //retourne la table de hachage créee
...
Key* compute_winner(CellProtected* decl, CellKey* candidates, CellKey* voters, int sizeC, int sizeV){
    //paramètres: la liste de vote(déclarations), la liste des candidats, la liste des votants, le nombre de candidats et le nombre de votants
    //retourne le gagnant
    //valeur de retour: la clé du gagnat
    verification_fraude(&decl); //vérifications des fraudes
